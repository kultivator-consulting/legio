...
RUN apk update
RUN apk add --no-cache bash ca-certificates git make vips-dev fftw-dev build-base
...
COPY go.mod .
COPY go.sum .
RUN CGO_ENABLED=1 GOOS=linux go build -ldflags '-w -s' -a -installsuffix cgo -o bin services/$SERVICE_NAME/*.go
...
RUN apk update
RUN apk --no-cache add ca-certificates tzdata vips-dev fftw-dev build-base
